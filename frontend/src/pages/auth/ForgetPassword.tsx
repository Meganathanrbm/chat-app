...
import { ForgetPasswordData } from "../../types/pages";

const ForgetPassword = () => {
  const { loading, forgetPassword } = useForgetPassword();
  const [showForgetPassword, setshowForgetPassword] = useState<boolean>(true);
  const {
    register,
    handleSubmit,
    formState: { errors },
  ...
  const onSubmit: SubmitHandler<ForgetPasswordData> = async (data) => {
    try {
      ...
      if (response.code === 200) setshowForgetPassword(false);
    } catch (error) {
