...
export const Profile = () => {
  const [isAboutEdit, setIsAboutEdit] = useState<boolean>(false);
  const { setViewProfile, viewProfile } = useConversation();
  const [aboutContent, setAboutContent] = useState<string>();
  const { onlineUsers } = useContext(SocketContext);
  const { authUser } = useContext(AuthContext);
  const isOnline = onlineUsers?.includes(viewProfile ? viewProfile._id : "");
  ...
  const handleAboutText: SubmitHandler<ProfileData> = (data) => {
    ...
    >
      ...
      <div className="px-5 sm:px-6 py-3 border-b dark:border-b-gray-800">
        <p className="text-slate-500 dark:text-slate-400  text-base mb-1">
          Since {formatDate(viewProfile ? viewProfile.createdAt : "")}
        </p>
      </div>
    </section>
  );
